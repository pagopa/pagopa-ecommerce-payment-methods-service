...
@Component
public class LangTypeIdFilterRule implements IFilterRule{
    @Override
    public boolean shouldExecute(String paymentInstrumentId, Integer amount, String language, String paymentTypeCode) {
        return checkQueryParam(paymentInstrumentId) && checkQueryParam(language) && checkQueryParam(paymentTypeCode)
                && amount == null;
    }
    ...
    public Flux<PspDocument> execute(PspRepository pspRepository, String paymentInstrumentId, Integer amount, String language, String paymentTypeCode) {
        return pspRepository.findPspMatchLangTypeId(language, paymentTypeCode, paymentInstrumentId);
    }
}
