...
class PaymentMethodsControllerTests {
    ...
    /*
     * @Test void shouldGetPaymentMethodPSPs() { String TEST_METHOD_ID =
     * UUID.randomUUID().toString(); PspDto pspDto = TestUtil.getTestPspDto();
     *
     * Mockito.when( paymentMethodService.retrievePaymentMethodById( any() )
     ...
     * PSPsResponseDto expectedResult = new PSPsResponseDto() .psp(List.of(pspDto));
     *
     * webClient .get() .uri("/payment-methods/" + TEST_METHOD_ID + "/psps")
     * .exchange() .expectStatus() .isOk() .expectBody(PSPsResponseDto.class)
     * .isEqualTo(expectedResult); }
     *
     * @Test void shouldPatchPaymentMethod() { UUID TEST_CAT = UUID.randomUUID();
     *
     * PaymentMethod paymentMethod = TestUtil.getPaymentMethod();
     *
     * Mockito.when( paymentMethodService.updatePaymentMethodStatus(
     * paymentMethod.getPaymentMethodID().value().toString(),
     * PaymentMethodStatusEnum.ENABLED ) ).thenReturn(Mono.just(paymentMethod));
     *
     * PaymentMethodResponseDto expectedResult =
     * TestUtil.getPaymentMethodResponse(paymentMethod);
     *
     * PaymentMethodRequestDto patchRequest = new PaymentMethodRequestDto()
     * .status(PaymentMethodRequestDto.StatusEnum.ENABLED);
     *
     * webClient .patch().uri("/payment-methods/" +
     * paymentMethod.getPaymentMethodID().value())
     * .contentType(MediaType.APPLICATION_JSON) .bodyValue(patchRequest) .exchange()
     * .expectStatus() .isOk() .expectBody(PaymentMethodResponseDto.class)
     * .isEqualTo(expectedResult); }
     *
     * @Test void shouldScheduleUpdate() {
     *
     * Mockito.when(afmClient.getPSPs(any(), any())).thenReturn(
     * Mono.just(TestUtil.getTestServices()) );
     *
     * webClient .put().uri("/payment-methods/psps") .exchange() .expectStatus()
     * .isAccepted();
     *
