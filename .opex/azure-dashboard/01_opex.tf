...
locals {
  name                = "${var.prefix}-${var.env_short}-opex_payment-methods"
        ...
        "0": {
          ...
          "metadata": {
            "inputs": [
              ...
              {
                "name": "PartSubTitle",
                "value": "/payment-methods/payment-methods",
                "isOptional": true
              },
              {
                "name": "Dimensions",
        ...
        "3": {
          ...
          "metadata": {
            "inputs": [
              ...
              {
                "name": "PartSubTitle",
                "value": "/payment-methods/payment-methods/{id}",
                "isOptional": true
              },
              {
                "name": "Dimensions",
        ...
        "6": {
          ...
          "metadata": {
            "inputs": [
              ...
              {
                "name": "Query",
                "value": "\nlet api_hosts = datatable (name: string) [\"api.platform.pagopa.it\"];\nlet threshold = 0.99;\nAzureDiagnostics\n| where originalHost_s in (api_hosts)\n| where requestUri_s matches regex \"/payment-methods/payment-methods/[^/]+/fees\"\n| summarize\n  Total=count(),\n  Success=count(httpStatus_d < 500) by bin(TimeGenerated, 5m)\n| extend availability=toreal(Success) / Total\n| project TimeGenerated, availability, watermark=threshold\n| render timechart with (xtitle = \"time\", ytitle= \"availability(%)\")\n",
                "isOptional": true
              },
              {
                "name": "ControlType",
              ...
              {
                "name": "PartSubTitle",
                "value": "/payment-methods/payment-methods/{id}/fees",
                "isOptional": true
              },
              {
                "name": "Dimensions",
        ...
        "7": {
          ...
          "metadata": {
            "inputs": [
              ...
              {
                "name": "Query",
                "value": "\nlet api_url = \"/payment-methods/payment-methods/[^/]+/fees\";\nlet api_hosts = datatable (name: string) [\"api.platform.pagopa.it\"];\nAzureDiagnostics\n| where originalHost_s in (api_hosts)\n| where requestUri_s matches regex api_url\n| extend HTTPStatus = case(\n  httpStatus_d between (100 .. 199), \"1XX\",\n  httpStatus_d between (200 .. 299), \"2XX\",\n  httpStatus_d between (300 .. 399), \"3XX\",\n  httpStatus_d between (400 .. 499), \"4XX\",\n  \"5XX\")\n| summarize count() by HTTPStatus, bin(TimeGenerated, 5m)\n| render areachart with (xtitle = \"time\", ytitle= \"count\")\n",
                "isOptional": true
              },
              {
                "name": "ControlType",
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alarm_availability_0" {
  name                = replace(join("_",split("/", "${local.name}-availability @ /payment-methods/payment-methods")), "/\\{|\\}/", "")
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  description             = "Availability for /payment-methods/payment-methods is less than or equal to 99% - ${local.dashboard_base_addr}${azurerm_portal_dashboard.this.id}"
  enabled                 = true
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alarm_time_0" {
  name                = replace(join("_",split("/", "${local.name}-responsetime @ /payment-methods/payment-methods")), "/\\{|\\}/", "")
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  data_source_id          = "/subscriptions/b9fc9419-6097-45fe-9f74-ba0641c91912/resourceGroups/pagopa-p-vnet-rg/providers/Microsoft.Network/applicationGateways/pagopa-p-app-gw"
  description             = "Response time for /payment-methods/payment-methods is less than or equal to 1s - ${local.dashboard_base_addr}${azurerm_portal_dashboard.this.id}"
  enabled                 = true
  auto_mitigation_enabled = false
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alarm_availability_1" {
  name                = replace(join("_",split("/", "${local.name}-availability @ /payment-methods/payment-methods/{id}")), "/\\{|\\}/", "")
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  data_source_id          = "/subscriptions/b9fc9419-6097-45fe-9f74-ba0641c91912/resourceGroups/pagopa-p-vnet-rg/providers/Microsoft.Network/applicationGateways/pagopa-p-app-gw"
  description             = "Availability for /payment-methods/payment-methods/{id} is less than or equal to 99% - ${local.dashboard_base_addr}${azurerm_portal_dashboard.this.id}"
  enabled                 = true
  auto_mitigation_enabled = false
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alarm_time_1" {
  name                = replace(join("_",split("/", "${local.name}-responsetime @ /payment-methods/payment-methods/{id}")), "/\\{|\\}/", "")
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  data_source_id          = "/subscriptions/b9fc9419-6097-45fe-9f74-ba0641c91912/resourceGroups/pagopa-p-vnet-rg/providers/Microsoft.Network/applicationGateways/pagopa-p-app-gw"
  description             = "Response time for /payment-methods/payment-methods/{id} is less than or equal to 1s - ${local.dashboard_base_addr}${azurerm_portal_dashboard.this.id}"
  enabled                 = true
  auto_mitigation_enabled = false
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alarm_availability_2" {
  name                = replace(join("_",split("/", "${local.name}-availability @ /payment-methods/payment-methods/{id}/fees")), "/\\{|\\}/", "")
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  data_source_id          = "/subscriptions/b9fc9419-6097-45fe-9f74-ba0641c91912/resourceGroups/pagopa-p-vnet-rg/providers/Microsoft.Network/applicationGateways/pagopa-p-app-gw"
  description             = "Availability for /payment-methods/payment-methods/{id}/fees is less than or equal to 99% - ${local.dashboard_base_addr}${azurerm_portal_dashboard.this.id}"
  enabled                 = true
  auto_mitigation_enabled = false
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alarm_time_2" {
  name                = replace(join("_",split("/", "${local.name}-responsetime @ /payment-methods/payment-methods/{id}/fees")), "/\\{|\\}/", "")
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  data_source_id          = "/subscriptions/b9fc9419-6097-45fe-9f74-ba0641c91912/resourceGroups/pagopa-p-vnet-rg/providers/Microsoft.Network/applicationGateways/pagopa-p-app-gw"
  description             = "Response time for /payment-methods/payment-methods/{id}/fees is less than or equal to 1s - ${local.dashboard_base_addr}${azurerm_portal_dashboard.this.id}"
  enabled                 = true
  auto_mitigation_enabled = false
